...
const dynamic_x = computed<number>({
  get: () => currentPipelineSettings.value.dynamic_x || 0,
  set: (value) => useCameraSettingsStore().changeCurrentPipelineSetting({ dynamic_x: value }, false),
});
const dynamic_y = computed<number>({
  get: () => currentPipelineSettings.value.dynamic_y || 0,
  set: (value) => useCameraSettingsStore().changeCurrentPipelineSetting({ dynamic_y: value }, false),
});
const dynamic_width = computed<number>({
  get: () => currentPipelineSettings.value.dynamic_width || frame_width.value,
  set: (value) => useCameraSettingsStore().changeCurrentPipelineSetting({ dynamic_width: value }, false),
});
const dynamic_height = computed<number>({
  get: () => currentPipelineSettings.value.dynamic_height || frame_height.value,
  set: (value) => useCameraSettingsStore().changeCurrentPipelineSetting({ dynamic_height: value }, false),
});
...
<template>
  <div v-if="currentPipelineSettings.pipelineType === PipelineType.ColoredShape">
    ...
    <pv-slider
      ...
      :slider-cols="interactiveCols"
      label="X"
      tooltip="The X coordinate of the top left corner of the dynamically cropped area"
      :min="0"
      :max="frame_width"
    />
    ...
    <pv-slider
      ...
      :slider-cols="interactiveCols"
      label="Y"
      tooltip="The Y coordinate of the top left corner of the dynamically cropped area"
      :min="0"
      :max="frame_height"
    />

    <pv-slider
      v-model="dynamic_width"
      :slider-cols="interactiveCols"
      label="Width"
      tooltip="The width of the dynamically cropped area"
      :min="1"
      :max="frame_width"
    />

    <pv-slider
      v-model="dynamic_height"
      :slider-cols="interactiveCols"
      label="Height"
      tooltip="The height of the dynamically cropped area"
      :min="1"
      :max="frame_height"
    />

    <pv-switch
      v-model="currentPipelineSettings.cropFallback"
      label="Fallback"
      tooltip="If the cropped area is not found, use the full image"
    />
  </div>
</template>
